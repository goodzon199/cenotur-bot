...
function resolveCity(input) {
    ...
    if (/^[A-Z]{3}$/.test(clean.toUpperCase()))
...
function setupBot(bot) {
    ...
    bot.help(async (ctx) => {
        await ctx.reply('🧳 *TravelBot — Помощь*\n\n'
            ...
            + 'Переход по ссылкам помогает проекту развиваться 🙌', { parse_mode: 'Markdown' });
    });
    ...
    bot.command('subscribe', async (ctx) => {
        userSessions.set(ctx.chat.id, { step: 'awaiting_origin' });
        await ctx.reply('📋 *Настройка мониторинга цен*\n\n'
            + 'Введите город *отправления* (например: Москва, LED):', { parse_mode: 'Markdown' });
    });
    bot.command('mysubs', async (ctx) => {
        const subs = (0, database_1.getSubscriptions)(ctx.chat.id);
        if (subs.length === 0) {
        ...
        const msg = subs
            .map((s, i) => (0, formatters_1.formatSubscriptionInfo)({
    ...
    bot.command('unsubscribe', async (ctx) => {
        ...
        const msg = subs
            ...
            .join('\n');
        await ctx.reply(`❌ Введите номер подписки для удаления:\n\n${msg}\n\n`
            + 'Или отправьте /cancel для отмены.');
        userSessions.set(ctx.chat.id, { step: 'awaiting_unsub_index' });
    });
    bot.hears(/^\/?cancel$/i, async (ctx) => {
        userSessions.delete(ctx.chat.id);
        await ctx.reply('❌ Отменено.', keyboards_1.mainKeyboard);
    });
    bot.on('text', async (ctx) => {
        const session = userSessions.get(ctx.chat.id);
        if (!session) {
            await ctx.reply('Используйте /flights для поиска или /help для помощи.', keyboards_1.mainKeyboard);
            return;
        }
        ...
        switch (session.step) {
            case 'awaiting_origin': {
                ...
                if (!code) {
                    await ctx.reply('❌ Не удалось распознать город. Попробуйте снова (например: Москва, LED):');
                    return;
                }
                session.origin = code;
            ...
            case 'awaiting_destination': {
                ...
                await ctx.reply('📅 Введите дату *отправления* в формате ДД.ММ.ГГГГ (например: 15.08.2026):', { parse_mode: 'Markdown' });
                break;
            }
            case 'awaiting_depart_date': {
                ...
                if (ctx.message.text?.toLowerCase().includes('subscribe')) {
                    ...
                    await ctx.reply('💰 Введите *максимальную цену* в рублях (например: 15000):', { parse_mode: 'Markdown' });
                }
                else {
                    session.step = 'awaiting_return_date';
                    await ctx.reply('📅 Если нужен *обратный билет*, введите дату возвращения (ДД.ММ.ГГГГ)\n'
                        + 'Или отправьте "нет" для поиска в одну сторону:', { parse_mode: 'Markdown' });
                }
                break;
            }
            ...
            case 'awaiting_price': {
                ...
                const sub = session;
                await ctx.reply('📋 *Подтверждение подписки:*\n\n'
                    + `✈️ ${CITY_NAMES[sub.origin] || sub.origin} → ${CITY_NAMES[sub.destination] || sub.destination}\n`
                    + `📅 ${sub.departDate}\n`
                    + `💰 до *${sub.maxPrice.toLocaleString('ru-RU')} RUB*\n\n`
                    + 'Я буду проверять цены каждый час и уведомлять о лучших предложениях.', { parse_mode: 'Markdown', ...(0, keyboards_1.confirmSubscriptionKeyboard)() });
                break;
            }
            case 'confirm_sub': {
                break;
            ...
            case 'awaiting_unsub_index': {
                ...
                (0, database_1.removeSubscription)(ctx.chat.id, index);
                userSessions.delete(ctx.chat.id);
                await ctx.reply(`✅ Подписка *${sub.originName} → ${sub.destinationName}* удалена.`, { parse_mode: 'Markdown' });
                break;
            }
            default:
                if (ctx.chat)
                    userSessions.delete(ctx.chat.id);
                await ctx.reply('Используйте /flights для поиска.', keyboards_1.mainKeyboard);
        }
    });
    function chatId(ctx) {
        if (!ctx.chat)
    ...
    bot.action('sub_confirm', async (ctx) => {
        ...
        (0, database_1.addSubscription)(cid, sub);
        userSessions.delete(cid);
        await ctx.editMessageText('✅ *Подписка создана!*\n\n'
            + `✈️ ${sub.originName} → ${sub.destinationName}\n`
            + `📅 ${sub.departDate}\n`
            + `💰 до *${sub.maxPrice.toLocaleString('ru-RU')} RUB*\n\n`
            + '🔔 Я буду уведомлять вас, когда цена упадёт ниже этого порога.', { parse_mode: 'Markdown' });
    });
    ...
    bot.action('new_search', async (ctx) => {
        const cid = chatId(ctx);
        if (cid)
            userSessions.set(cid, { step: 'awaiting_origin' });
    ...
    bot.action('my_subs', async (ctx) => {
        ...
        if (!cid)
            return;
        const subs = (0, database_1.getSubscriptions)(cid);
        if (subs.length === 0) {
            await ctx.reply('📋 У вас нет активных подписок.');
            return;
        }
        const msg = subs
            .map((s, i) => (0, formatters_1.formatSubscriptionInfo)({ ...s, index: i }))
            .join('\n\n');
        await ctx.reply(`📋 *Ваши подписки:*\n\n${msg}`, { parse_mode: 'Markdown' });
    });
}
async function performFlightSearch(ctx, session) {
    ...
    try {
        ...
        if (flights.length === 0) {
            await ctx.reply('😔 Билеты не найдены. Попробуйте:\n'
                + '• Другие даты\n'
                + '• Другое направление\n'
                + '• /flights — новый поиск', (0, keyboards_1.searchAgainKeyboard)());
            return;
        }
        const msg = (0, formatters_1.formatFlightsList)(flights.slice(0, 5), `${CITY_NAMES[session.origin] || session.origin} → ${CITY_NAMES[session.destination] || session.destination}`);
        await ctx.reply(msg, { parse_mode: 'Markdown', link_preview_options: { is_disabled: true } });
        await ctx.reply('💎 *Партнёрские ссылки:* переход по ссылкам помогает проекту.\n'
            + 'Вы можете купить билет по самой низкой цене!', { parse_mode: 'Markdown', ...(0, keyboards_1.searchAgainKeyboard)() });
    }
    catch (error) {
...
async function performHotelSearch(ctx, session) {
    ...
    try {
        const hotels = await (0, hotels_1.searchHotels)({
            location: session.destination,
            checkIn: session.departDate,
            checkOut: session.returnDate,
        });
        if (hotels.length === 0) {
            await ctx.reply('😔 Отели не найдены. Попробуйте другие даты или направление.', (0, keyboards_1.searchAgainKeyboard)());
            return;
        }
        const msg = (0, formatters_1.formatHotelsList)(hotels, `📍 Отели в ${CITY_NAMES[session.destination] || session.destination}`);
        await ctx.reply(msg, { parse_mode: 'Markdown', link_preview_options: { is_disabled: true } });
        await ctx.reply('💎 Переход по ссылкам помогает проекту. Спасибо! 🙌', { ...(0, keyboards_1.searchAgainKeyboard)() });
    }
    catch (error) {
